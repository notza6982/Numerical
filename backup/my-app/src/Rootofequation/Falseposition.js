...
class Falseposition extends Component{
	data() {
		axios.get('http://localhost:4000/falseposition')
		  .then(res => {
		  ...
		  this.forceUpdate();
		  this.falseposi(Number(this.state.xl),Number(this.state.xr));
	...
	constructor() {
		...
		this.falseposi = this.falseposi.bind(this);
	...
	falseposi(L,R){
		...
		data['x'] = []
		data['error'] = []
		 m1 = ((L*this.func(R))-(R*this.func(L)))/this.func(R)-this.func(L);
		 data['xl'][0] = L
		data['xr'][0] = R
		data['x'][0] = m1
		data['error'][0] = "-"
		 ...
		 do{
		 	 console.log("Iteration:",i)
   		 	 m2 = ((L*this.func(R))-(R*this.func(L)))/this.func(R)-this.func(L);
    ...
    return(
      ...
      <Leftmenu/>
		  ...
		  <h1 style={{textAlign: 'center' }}><Text underline type="warning" >Falseposition</Text></h1>    
          </Header>
          <Content
            style={{
              margin: '0px 0px',
    ...
    <FormControl aria-label="Small" aria-describedby="inputGroup-sizing-sm" placeholder="43*x-1" name="fx" value={this.state.fx} />
  </InputGroup>
    ...
    <InputNumber placeholder="0.02" type='Number' name="xl" defaultValue={this.state.xl}  /> 

    <Col span={1} />
	<InputGroup.Prepend>
      <InputGroup.Text id="inputGroup-sizing-sm">XR</InputGroup.Text>
    </InputGroup.Prepend>
    <InputNumber placeholder="0.03" type='Number'  name="xr" defaultValue={this.state.xr}  /> 
	
  </InputGroup>
      <h1 style={{textAlign: 'center' }}><Button type="primary" size='large' onClick= { ()=>this.falseposi(Number(this.state.xl),Number(this.state.xr))}>
	  <Link to="/Rootofequation/Falseposition">Submit</Link></Button></h1>
