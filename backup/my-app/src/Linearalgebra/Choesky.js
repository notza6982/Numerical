...
class Choesky extends Component{
	constructor() {
        super();
        this.state = {
            Row: "",
    ...
    return(
      ...
      <Leftmenu/>
       ...
       <h1 style={{textAlign: 'center' }}><Text underline type="warning" >Choesky</Text></h1>
  ...
  <Button type="primary" size='large' onClick= { ()=>this.createMatrix(Number(this.state.Row),Number(this.state.Column))}>
	  <Link to="/Linearalgebra/Choesky">OK</Link></Button> </div>}
  
      
      {this.state.showMatrixForm && <div><h2>Matrix [A]</h2><br/>{matrixA}<h2>Vector [B]<br/></h2>{matrixB}
      <br />
      <h1 style={{textAlign: 'center' }}><Button type="primary" size='large' onClick= { ()=>this.cho()}>
	  <Link to="/Linearalgebra/Choesky">Submit</Link></Button></h1>
      </div>
      }
      </form>
