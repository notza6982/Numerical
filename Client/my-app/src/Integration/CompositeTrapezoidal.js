...
class CompositeTrapezoidal extends Component{
	constructor() {
	 ...
	 trap(a,b,n){
      var h = (b-a)/n;
      var I = (h / 2) * (this.func(a) + this.func(b) + 2*this.summationFunction(n, h));
  ...
  summationFunction(start, n, h) {
    ...
    return(
      ...
      <Leftmenu/>
       ...
       <h1 style={{textAlign: 'center' }}><Text underline type="warning" >Composite Trapezoidal</Text></h1>
    ...
    <InputNumber placeholder="8" type='Number'  name="n" defaultValue={this.state.n}  /> 
      ...
      <h1 style={{textAlign: 'center' }}><Button type="primary" size='large' onClick= { ()=>this.trap(Number(this.state.a),Number(this.state.b),Number(this.state.n))}>
	  <Link to="/Integration/CompositeTrapezoidal">Submit</Link></Button></h1>
      </form>
	  <br /> 
