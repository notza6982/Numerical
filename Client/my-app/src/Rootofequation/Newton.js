...
class Newton extends Component{
  data() {
		axios.get('http://localhost:4000/newthon')
		  .then(res => {
		  ...
		  this.forceUpdate();
		  this.newton(Number(this.state.x0));
    ...
    data['error'][0] = "-"
		 ...
		 var i=1;
		 do{
            data['x'][i] = data['x'][i-1]-(this.func(data['x'][i-1])/this.funcDiff(data['x'][i-1]));
            data['error'][i] = (data['x'][i]-data['x'][i-1])/2
    		 i++;
  ...
  <InputGroup size="sm" className="mb-3">
    ...
    <InputNumber placeholder="2" type='Number'  name="x0" defaultValue={this.state.x0}  /> 
      ...
      <h1 style={{textAlign: 'center' }}><Button type="primary" size='large' onClick= { ()=>this.newton(Number(this.state.x0))}>
	  <Link to="/Rootofequation/Newton">Submit</Link></Button></h1>
