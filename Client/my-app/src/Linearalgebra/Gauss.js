...
class Gauss extends Component{
	constructor() {
	 ...
	 gau(){
        ...
        var X = new Array(n);
        X[n-1] = round(B[n-1] / A[n-1][n-1]); //find Xn
    ...
    return(
      ...
      <Leftmenu/>
       ...
       <h1 style={{textAlign: 'center' }}><Text underline type="warning" >Gauss Elimination</Text></h1>
  ...
  <Button type="primary" size='large' onClick= { ()=>this.createMatrix(Number(this.state.Row),Number(this.state.Column))}>
	  <Link to="/Linearalgebra/Gauss">OK</Link></Button> </div>}
  
      
      {this.state.showMatrixForm && <div><h2>Matrix [A]</h2><br/>{matrixA}<h2>Vector [B]<br/></h2>{matrixB}
      <br />
      <h1 style={{textAlign: 'center' }}><Button type="primary" size='large' onClick= { ()=>this.gau()}>
	  <Link to="/Linearalgebra/Gauss">Submit</Link></Button></h1>
      </div>
      }
      </form>
